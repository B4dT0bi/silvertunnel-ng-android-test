 ...
 * @author Tobias Boese
 */
public class AuthorityKeyCertificatesLocalTest extends InstrumentationTestCase {
    /** */
    ...
    static {        // install BC, if not already done
     ...
     * @throws Exception
     */
    public void testSingeCertificateSignatureCheck() throws IOException {
        // parse certificate, must throw TorException
        final String allCertsStr = FileUtil.readFileFromInputStream(getInstrumentation().getTargetContext().getResources().openRawResource(R.raw.example_authority_key_corrupted));
        try {
    ...
    public void testAllCertificatesSuccessfulParsing() throws Exception {
        ...
        // check result
        assertEquals("list: invalid number of certificates in allCerts", 8, all.size());
    ...
    public void testSkippingOfCertificatedWithUnauthorizedFingerprint() throws Exception {
        ...
        // is valid
        final String allCertsStr = FileUtil.readFileFromInputStream(getInstrumentation().getTargetContext().getResources().openRawResource(R.raw.unauthorized_authority_key));
